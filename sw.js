...
async function checkUrssafDeclaration(p, templates) {
  try {
    ...
    // Une déclaration ne compte que si l'attestation est jointe (voir
    // getDerniereDeclaration côté index.html — à garder en phase).
    const declarations = (p.urssafDeclarations || []).filter(d => d.attestationBase64);
